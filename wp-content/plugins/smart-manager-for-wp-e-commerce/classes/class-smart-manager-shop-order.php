...
if ( ! class_exists( 'Smart_Manager_Shop_Order' ) ) {
	class Smart_Manager_Shop_Order extends Smart_Manager_Base {
		...
		public static function generate_orders_custom_column_model( $column_model ) {
			...
			$custom_columns = array( 'shipping_method', 'coupons_used', 'line_items', 'details', 'order_sub_total', 'edit_link' );
		...
		public static function generate_data_model( $data_model = array(), $args = array() ) {
			...
			if ( ! empty( Smart_Manager::$sm_is_woo79 ) ) {
				...
				if( ! empty( $curr_obj->req_params['search_text'] ) ) {
					...
					// Code for custom table simple search.
					$woi_order_ids = $wpdb->get_col(
						...
						if ( ! empty( $woi_order_ids ) && is_array( $woi_order_ids ) ) {
							$orders_queries['field_query'][] = array(
								'field'     => $wpdb->prefix . 'wc_orders.id',
								'value'     => implode( ',', $woi_order_ids ),
								'compare'   => 'IN',
							);
						}
        ...
        */
		...
		public static function process_shipping_details_update( $args = array() ) {
			...
			foreach( $args['edited_data'] as $id => $edited_row ) {
				...
				foreach( $edited_row as $key => $value ) {
					...
					if ( ! in_array( $update_column, array( '_order_shipping', 'shipping_total_amount', 'shipping_method' ) ) ) {
						continue;
					}
		...
		/**
		 * Function to modidy join clause.
		 *
		...
		public function sm_order_query_join( $join = '', $wp_query_obj = null ) {
			global $wpdb;
			if ( ( false !== strpos( $join, 'woocommerce_order_items' ) ) || ( false !== strpos( $join, 'woocommerce_order_itemmeta' ) ) || empty( $this->req_params['search_text'] ) || empty( $this->dashboard_key ) ) {
				return $join;	
			}
			$join .= " LEFT JOIN {$wpdb->prefix}woocommerce_order_items AS oi ON ({$wpdb->prefix}posts.id = oi.order_id AND {$wpdb->prefix}posts.post_type = '{$this->dashboard_key}') LEFT JOIN {$wpdb->prefix}woocommerce_order_itemmeta AS oim ON (oi.order_item_id = oim.order_item_id AND oi.order_item_type = 'shipping')";
			return $join;
		}
	}
}
