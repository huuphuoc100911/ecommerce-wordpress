...
add_action('woocommerce_before_single_product_summary', function () {
	...
	$checkCountAttribute = count($product->get_attributes()) === count($product->get_default_attributes());

	if (!empty($product->get_default_attributes())) {
		$checkDefaultAttributeExit = false;
		$arrayDefaultAttributes = [];

		foreach ($product->get_default_attributes() as $key => $value) {
			if (ctype_digit($value)) {
				$checkDefaultAttributeExit = true;
			} else {
				$arrayDefaultAttributes[$key] = $value;
			}
		}

		$checkCountAttribute = count($product->get_attributes()) === count($arrayDefaultAttributes);

		if ($checkDefaultAttributeExit || !$checkCountAttribute) {
			$product->set_default_attributes($arrayDefaultAttributes);
			$checkCountAttribute = count($product->get_attributes()) === count($product->get_default_attributes());
		}
	}

	if (!empty($product->get_attributes()) && !$checkCountAttribute) {
		...
		foreach ($product->get_attributes() as $key => $attribute) {
			...
			if (!array_key_exists($attribute['name'], $default_attribute)) {
				$arrayCheck = [];
				foreach ($options as $option) {
					$tag_slug = get_term($option, $key)->slug;
					array_push($arrayCheck, $tag_slug);
				}

				if ($key == "pa_size") {
					if (in_array("small", $arrayCheck)) {
						$default_attribute[$key] = "small";
					} else {
						$default_attribute[$key] = get_term($options[0], $key)->slug;;
					}
				} else {
					sort($arrayCheck);
					$default_attribute[$key] = $arrayCheck[0];
				}
			}
		}
		...
		update_post_meta($product->get_id(), '_default_attributes', $default_attribute);
	}
});
